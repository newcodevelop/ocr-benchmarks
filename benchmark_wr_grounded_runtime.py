#!/usr/bin/env python3
"""
WildReceipt benchmark inference with OCR-nudged semantic grounding.

This file is the WildReceipt counterpart of ``benchmark_sroie_grounded.py``.
The model-serving stack, value-to-OCR matching, local/global bbox recovery,
primitive OCR proposal, crop verification, output files, and metric families
remain the same.  Dataset-specific work is isolated to:

1. Reading the official WildReceipt JSON-lines annotations.
2. Converting numeric labels through ``class_list.txt`` instead of hard-coding
   a label order that may differ between WildReceipt releases/converters.
3. Representing every semantic category as a LIST of occurrences.  This is
   necessary because product item, quantity, and price fields repeat.
4. Evaluating all 24 key/value categories.  ``others`` is intentionally not an
   extraction target because it is the background/non-key-information class.

The most important anti-leakage rule in this script is:

    Ground truth receives (text, bbox, semantic label).
    OCR grounding receives only (text, bbox).

The official annotations can therefore be used as an OCR transcription source,
but the semantic label is stripped before candidate generation.  For a stricter
realistic experiment, point ``--ocr_dir`` at predictions from an external OCR
engine; the remaining methodology is unchanged.

Official WildReceipt layout after extracting ``wildreceipt.tar``:

    dataset_root/
      class_list.txt
      train.txt                 # one JSON document per line
      test.txt                  # one JSON document per line
      image_files/...           # paths referenced by file_name

Core principle, retained from the SROIE implementation:

    The VLM predicts WHAT a field contains.
    OCR evidence retrieval determines WHERE that value occurs.
...
def apply_model_preset(args: argparse.Namespace) -> None:
    ...
    if args.spawn_vllm and not preset.get("spawn_vllm_supported", True) and not model_overridden:
        raise ValueError(
    ...
    if args.output_json is None:
        args.output_json = f"WildReceipt_{slug}_grounded_results.json"
    if args.output_csv is None:
        args.output_csv = f"WildReceipt_{slug}_grounded_results.csv"
    if args.grounding_csv is None:
        args.grounding_csv = f"WildReceipt_{slug}_grounding_details.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model_preset",
        default=os.environ.get("MODEL_PRESET", "granite-vision-3.3-2b"),
        help=(
...
def field_semantic_score(text: str, field: str, start: int = 0, num_lines: int = 1, total_lines: int = 1) -> float:
    ...
    if field_is_key(field):
        ...
        if uppercase_ratio > 0.55 and len(tokens) >= 2:
            score += 0.20
        if 1 <= len(tokens) <= 8:
            score += 0.10
...
def main() -> None:
    ...
    base_runtime_seconds: List[float] = []
    proposed_runtime_seconds: List[float] = []
    ...
    for idx, record in enumerate(records):
        ...
        for field in EVAL_FIELDS:
            predicted = extract_field_predictions(pred_json, field, sent_size, original_size)
        ...
        proposed_elapsed = base_elapsed + grounding_elapsed
        base_runtime_seconds.append(base_elapsed)
        proposed_runtime_seconds.append(proposed_elapsed)
        ...
        result = {
            ...
            "runtime_seconds": {
                "base_model": base_elapsed,
                "proposed": proposed_elapsed,
            },
    ...
    print_runtime_summary(base_runtime_seconds, proposed_runtime_seconds)
...
def print_runtime_summary(base_runtimes: Sequence[float], proposed_runtimes: Sequence[float]) -> None:
    base_avg = sum(base_runtimes) / len(base_runtimes) if base_runtimes else 0.0
    proposed_avg = sum(proposed_runtimes) / len(proposed_runtimes) if proposed_runtimes else 0.0
    print("\n====================================================")
    print("AVERAGE RUNTIME")
    print("====================================================")
    print(f"Base model average runtime : {base_avg:.4f} seconds/image")
    print(f"Proposed average runtime   : {proposed_avg:.4f} seconds/image")
    print("====================================================\n")
